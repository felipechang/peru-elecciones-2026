"""Environment-backed configuration for the researcher package."""
from __future__ import annotations

import math
import os
from dataclasses import dataclass
from urllib.parse import urlparse


class ConfigError(ValueError):
    """Raised when a required environment variable is missing or invalid."""


def _env_str(name: str, default: str) -> str:
    raw = os.environ.get(name)
    if raw is None:
        return default
    stripped = raw.strip()
    return default if stripped == "" else stripped


def _env_float(name: str, default: float) -> float:
    raw = os.environ.get(name)
    if raw is None or raw.strip() == "":
        return default
    try:
        value = float(raw.strip())
    except ValueError as exc:
        raise ConfigError(
            f"{name} must be a number, got {raw!r}"
        ) from exc
    if not math.isfinite(value) or value <= 0:
        raise ConfigError(
            f"{name} must be a positive finite number, got {value!r}"
        )
    return value


def _env_int(name: str, default: int) -> int:
    raw = os.environ.get(name)
    if raw is None or raw.strip() == "":
        return default
    try:
        value = int(raw.strip())
    except ValueError as exc:
        raise ConfigError(
            f"{name} must be an integer, got {raw!r}"
        ) from exc
    if value <= 0:
        raise ConfigError(
            f"{name} must be a positive integer, got {value!r}"
        )
    return value


def _validate_http_url(name: str, url: str) -> str:
    if not url:
        raise ConfigError(f"{name} must be a non-empty URL")
    parsed = urlparse(url)
    if parsed.scheme not in {"http", "https"}:
        raise ConfigError(
            f"{name} must use http or https scheme, got {url!r}"
        )
    if not parsed.netloc:
        raise ConfigError(f"{name} must include a host, got {url!r}")
    return url.rstrip("/")


def _optional_env_str(name: str) -> str | None:
    raw = os.environ.get(name)
    if raw is None or raw.strip() == "":
        return None
    return raw.strip()


@dataclass(frozen=True)
class ResearcherConfig:
    openrouter_base_url: str
    openrouter_api_key: str
    openrouter_http_referer: str | None
    openrouter_app_title: str | None
    summarization_model: str
    openrouter_timeout: float
    searxng_url: str
    searxng_timeout: float
    # Maximum number of search results to feed into the summarizer per candidate
    max_results: int
    # How many seconds to wait between candidates to avoid hammering SearXNG
    request_delay: float
    # Interval in seconds between full pipeline runs (default: 3600 = 1 hour)
    schedule_interval: int


def load_researcher_config() -> ResearcherConfig:
    """Read and validate configuration from the process environment."""
    api_key = _env_str("OPENROUTER_API_KEY", "")
    if not api_key:
        raise ConfigError("OPENROUTER_API_KEY is required")
    or_base = _env_str("OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")
    searxng_raw = _env_str("SEARXNG_URL", "http://localhost:8080")
    model = _env_str("SUMMARIZATION_MODEL", "openai/gpt-5.4-nano")

    return ResearcherConfig(
        openrouter_base_url=_validate_http_url("OPENROUTER_BASE_URL", or_base),
        openrouter_api_key=api_key,
        openrouter_http_referer=_optional_env_str("OPENROUTER_HTTP_REFERER"),
        openrouter_app_title=_optional_env_str("OPENROUTER_APP_TITLE"),
        summarization_model=model,
        openrouter_timeout=_env_float("OPENROUTER_TIMEOUT", 120.0),
        searxng_url=_validate_http_url("SEARXNG_URL", searxng_raw),
        searxng_timeout=_env_float("SEARXNG_TIMEOUT", 30.0),
        max_results=_env_int("RESEARCHER_MAX_RESULTS", 5),
        request_delay=_env_float("RESEARCHER_REQUEST_DELAY", 1.0),
        schedule_interval=_env_int("RESEARCHER_SCHEDULE_INTERVAL", 3600),
    )


_config: ResearcherConfig | None = None


def get_config() -> ResearcherConfig:
    """Return the validated singleton config (loaded on first call)."""
    global _config
    if _config is None:
        _config = load_researcher_config()
    return _config
